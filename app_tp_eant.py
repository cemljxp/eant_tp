...
app.layout = html.Div([
    ...
    dbc.Row([
        dbc.Col(
            dcc.Tabs([
                dcc.Tab(label='Estimación de MW', style=tab_style, selected_style=tab_selected_style,
                        children=[
...
def update_predict(temp_avg, temp_max, temp_min, wind_avg, hum, hpa, sw_holiday, date_pick):
    ...
    if sw_holiday == True:
        ...
        val_pred[0][0] = 0
    val_pred[0][1] = round(temp_avg,0)
    val_pred[0][2] = round(temp_max,0)
    val_pred[0][3] = round(temp_min,0)
    val_pred[0][4] = hpa
    val_pred[0][5] = hum
    ...
    return ['Temperatura Promedio ({:.1f} °C)'.format(temp_avg), temp_max_min, temp_min_max, 'Temperatura Máxima ({:.1f} °C)'.format(temp_max), 'Temperatura Mínima ({:.1f} °C)'.format(temp_min), 'Velocidad de Viento Promedio ({} km/h)'.format(wind_avg), 'Humedad Relativa ({}%) '.format(hum), 'Presión Atmosférica ({} hPa) '.format(hpa), led_val, p_MW_max]
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
@app.callback(
